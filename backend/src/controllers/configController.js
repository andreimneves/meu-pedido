    ...
    async atualizarConfiguracoes(req, res) {
        try {
            ...
            if (d.tipo_atualizacao === 'geral') {
                if (existe.rows.length > 0) {
                    await pool.query(
                        ...
                        [
                            ...
                            d.mensagem_banner_cor || '#FFF3E0', d.mensagem_banner_texto || '#E65100', d.mensagem_banner_icone || '📢', d.logo_url || '', tenantId
                        ]
                    );
                } else {
                    await pool.query(
                ...
                return res.json({ mensagem: 'Configurações Gerais salvas com sucesso!' });
            }

            return res.status(400).json({ erro: 'Etiqueta de atualização inválida.' });
